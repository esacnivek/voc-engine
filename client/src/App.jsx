...
// ---------------------------------------------------------------------------
// Proxy base URL — set VITE_PROXY_URL in .env to your ngrok URL when dev-ing
// e.g.  VITE_PROXY_URL=https://xxxx.ngrok-free.app
// Falls back to localhost for local-only dev without ngrok
// ---------------------------------------------------------------------------
const PROXY_BASE =
...
export default function App() {
  ...
  const analyze = async () => {
    ...
    // -----------------------------------------------------------------------
    // Real Zendesk mode — credentials go to the proxy (POST body, not URL)
    // The proxy handles the Zendesk API call server-side, avoiding CORS errors
    // -----------------------------------------------------------------------
    if (!useDemo) {
      try {
        setProgress(25);
        ...
        let proxyRes;
        try {
          proxyRes = await fetch(`${PROXY_BASE}/proxy/tickets`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            // Credentials stay in the POST body — never in the URL
            body: JSON.stringify({
          ...
          throw new Error(
            `Could not reach the proxy server at ${PROXY_BASE}. ` +
            `Make sure proxy-server.js is running (node proxy-server.js) ` +
            `and VITE_PROXY_URL is set correctly in your .env file. ` +
            `If using ngrok, confirm the tunnel is active.`
          );
        }
        ...
        if (!proxyRes.ok) {
          throw new Error(proxyData.error || `Proxy returned HTTP ${proxyRes.status}.`);
        }

        if (!proxyData.tickets?.length) {
          throw new Error("Zendesk returned no tickets. Your account may have no tickets yet.");
        }

        tickets = proxyData.tickets;
        setProgressLabel(`Fetched ${tickets.length} tickets — sending to Claude...`);
      } catch (e) {
        setError(e.message);
        setScreen("setup");
        return;
...
Return ONLY valid JSON (no markdown) with this exact structure:
{
  ...
  const urgencyColor = (u) =>
    u === "high" ? "#ff4d4d" : u === "medium" ? "#ffaa00" : "#00cc88";
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#e8e4dc", fontFamily: "Georgia, serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .proxy-badge { display:inline-flex; align-items:center; gap:6px; padding:4px 10px;
          border-radius:999px; border:1px solid #1e3a1e; background:#0a1a0a;
          font-family:'DM Mono',monospace; font-size:10px; color:#00cc88; letter-spacing:0.08em; }
        .proxy-badge .dot { width:5px; height:5px; border-radius:50%; background:#00cc88;
          box-shadow:0 0 6px #00cc88; animation: pulse 2s ease-in-out infinite; }
      ...
      <div style={{ borderBottom: "1px solid #1a1a2e", padding: "18px 36px", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span className="mono" style={{ fontSize: 12, color: "#c8a96e", letterSpacing: "0.15em" }}>
          VOICE OF CUSTOMER ENGINE
        </span>
        <span style={{ marginLeft: "auto", fontSize: 11, color: "#2a2a3e", fontFamily: "monospace" }}>
          Powered by Claude
        </span>
      </div>
      ...
      {screen === "setup" && (
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "56px 24px" }} className="fadeUp">
          ...
          <p style={{ color: "#555", fontSize: 15, lineHeight: 1.7, marginBottom: 48 }}>
            Drop in your Zendesk tickets and get executive-grade insights, sentiment
            analysis, and auto-drafted FAQs — powered by Claude.
          </p>

          {/* Error banner */}
          {error && (
            <div style={{
              background: "#1a0808", border: "1px solid #ff4d4d33", borderRadius: 6,
              padding: "12px 16px", marginBottom: 24, color: "#ff6b6b", fontSize: 13,
          ...
          <div style={{ marginBottom: 28 }}>
            <label className="mono" style={{ fontSize: 10, color: "#c8a96e", letterSpacing: "0.12em", display: "block", marginBottom: 10 }}>
              ANTHROPIC API KEY *
            </label>
            <input
              type="password"
              ...
              placeholder="sk-ant-..."
              style={{
                width: "100%", border: "1px solid #1e1e30", borderRadius: 6,
                padding: "12px 14px", color: "#e8e4dc", fontSize: 14,
                fontFamily: "monospace", background: "#0d0d1a",
              }}
            />
          </div>
          ...
          {/* Data source toggle */}
          <div style={{ marginBottom: 28 }}>
            <label className="mono" style={{ fontSize: 10, color: "#c8a96e", letterSpacing: "0.12em", display: "block", marginBottom: 12 }}>
              DATA SOURCE
            </label>
          ...
          {/* Zendesk credentials — only shown when "My Zendesk" is selected */}
          {!useDemo && (
            <div style={{
              padding: "20px", background: "#0d0d1a",
              border: "1px solid #1e1e30", borderRadius: 8, marginBottom: 28,
            }}>

              {/* Proxy status indicator */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <span className="mono" style={{ fontSize: 10, color: "#333", letterSpacing: "0.08em" }}>
                  ZENDESK CREDENTIALS
                </span>
                <span className="proxy-badge">
                  <span className="dot" />
                  VIA PROXY · {PROXY_BASE.replace("http://", "").replace("https://", "")}
                </span>
              </div>

              <div style={{
                background: "#08100d", border: "1px solid #0d2a1a", borderRadius: 6,
                padding: "10px 12px", marginBottom: 18,
                fontFamily: "monospace", fontSize: 11, color: "#2d6b45", lineHeight: 1.6,
              }}>
                🔒 Credentials are sent to the local proxy via POST body — never exposed in URLs or browser network logs.
              </div>

              {[
                { label: "SUBDOMAIN",  ph: "yourcompany (not the full URL)", val: zdSubdomain, set: setZdSubdomain, pwd: false },
                { label: "EMAIL",      ph: "admin@yourcompany.com",          val: zdEmail,     set: setZdEmail,     pwd: false },
                { label: "API TOKEN",  ph: "Your Zendesk API token",         val: zdToken,     set: setZdToken,     pwd: true  },
              ].map(f => (
                <div key={f.label} style={{ marginBottom: 16 }}>
                  <label className="mono" style={{ fontSize: 10, color: "#444", letterSpacing: "0.1em", display: "block", marginBottom: 7 }}>
                    {f.label}
                  </label>
                  <input
                    ...
                    placeholder={f.ph}
                    style={{
                      width: "100%", border: "1px solid #1a1a2e", borderRadius: 4,
                      padding: "9px 12px", color: "#e8e4dc", fontSize: 13,
                      fontFamily: "monospace", background: "#08080f",
                    }}
                  />
                </div>
              ))}
              ...
              <div style={{
                ...
                fontFamily: "monospace", fontSize: 11, color: "#333", lineHeight: 1.7,
              }}>
                <span style={{ color: "#555" }}>Need an API token?</span>
                {" "}Admin Center → Apps &amp; Integrations → Zendesk API → Add API token
              </div>
            </div>
          )}
          ...
          {useDemo && (
            <p className="mono" style={{ fontSize: 11, color: "#2a2a3e", marginTop: 10, textAlign: "center" }}>
              10 pre-loaded sample tickets — no Zendesk account needed
            </p>
          )}
        </div>
      )}
      ...
      {screen === "analyzing" && (
        <div style={{ maxWidth: 440, margin: "0 auto", padding: "100px 24px", textAlign: "center" }}>
          <div style={{
            ...
            border: "2px solid #1e1e30", borderTopColor: "#c8a96e",
            margin: "0 auto 28px", animation: "spin 1s linear infinite",
          }} />
          ...
          <div style={{ height: 2, background: "#1e1e30", borderRadius: 1 }}>
            <div style={{
              height: "100%", background: "#c8a96e", borderRadius: 1,
      ...
      {screen === "results" && insights && (
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "36px 24px 80px" }} className="fadeUp">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 20 }}>
            {[
              ...
              <div key={m.label} style={{
                ...
                <div className="mono" style={{ fontSize: 10, color: "#333", marginTop: 8, letterSpacing: "0.08em" }}>
                  {m.label.toUpperCase()}
                </div>
              </div>
            ))}
          ...
          {[
            {
              ...
              content: (
                <p style={{ fontSize: 16, lineHeight: 1.75, color: "#aaa8a0", fontStyle: "italic" }}>
              ...
              content: insights.top_issues?.map((issue, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    borderBottom: i < insights.top_issues.length - 1 ? "1px solid #111120" : "none",
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                      ...
                      <span style={{
                        ...
                        background: uc + "22", color: uc, border: `1px solid ${uc}44`,
                      }}>{issue.urgency}</span>
                    </div>
                    {issue.quotes?.[0] && (
                      <p className="mono" style={{ fontSize: 11, color: "#333", fontStyle: "italic", lineHeight: 1.5 }}>
                        "{issue.quotes[0]}"
                      </p>
                    )}
                  </div>
                ...
                return (
                  <div key={i} style={{ display: "flex", gap: 14, marginBottom: 16 }}>
                    ...
                    <span style={{
                      ...
                      fontSize: 10, fontFamily: "monospace", textTransform: "uppercase",
                      height: "fit-content", background: uc + "22", color: uc, border: `1px solid ${uc}44`,
                    }}>{t.urgency}</span>
                  </div>
                );
              }),
            },
            {
              ...
              content: insights.suggested_faqs?.map((faq, i) => (
                <div key={i} style={{
                  ...
                  borderBottom: i < insights.suggested_faqs.length - 1 ? "1px solid #111120" : "none",
                }}>
                  <div style={{ fontSize: 14, color: "#ddd", marginBottom: 6, fontWeight: "bold" }}>
                    Q: {faq.question}
                  </div>
                  <div style={{ fontSize: 12, color: "#555", lineHeight: 1.6, fontFamily: "monospace" }}>
                    A: {faq.answer}
                  </div>
                </div>
              )),
            ...
            <div key={section.title} style={{
              ...
              <div className="mono" style={{ fontSize: 10, color: "#c8a96e", letterSpacing: "0.14em", marginBottom: 18 }}>
                {section.title}
              </div>
              {section.content}
            </div>
          ...
          <button
            onClick={() => { setScreen("setup"); setInsights(null); setError(""); }}
            style={{
